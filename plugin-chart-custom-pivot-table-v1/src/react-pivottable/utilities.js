        ...
        return {
          ...
          value(x) {
            switch (this.record['Метрика'] || x) {
              ...
              case '% корректных ответов в срок':
                return (
                  this.pctCorrectResponsesOnTime.sum /
                  this.pctCorrectResponsesOnTime.count
                );
              case '% ответов в срок':
                return (
                  this.pctResponsesOnTime.sum / this.pctResponsesOnTime.count
                );
              case '% ответов на запросы':
  ...
  processRecord(record) {
    ...
    for (let ci = colStart; ci <= colKey.length; ci += 1) {
      ...
      if (!this.colTotals[flatColKey]) {
        ...
        this.colTotals[flatColKey] = this.getCustomAggregator(
          record,
          colKey,
        )(this, [], fColKey);
      }
    ...
    for (let ri = rowStart; ri <= rowKey.length; ri += 1) {
      ...
      for (let ci = colStart; ci <= colKey.length; ci += 1) {
        ...
        if (!this.tree[flatRowKey][flatColKey]) {
          if (isRowSubtotal) {
            this.tree[flatRowKey][flatColKey] = this.getCustomAggregator(
              record,
